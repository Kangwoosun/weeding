...
const ATTENDANCE_NOTICE_DISMISSED_KEY =
  "weddingInvitation.attendanceNoticeDismissed"
const ATTENDANCE_SUBMITTED_KEY = "weddingInvitation.attendanceSubmitted"
...
export const AttendanceInfo = () => {
  ...
  const [hasSubmittedAttendance, setHasSubmittedAttendance] = useState(() =>
    getStoredFlag(ATTENDANCE_SUBMITTED_KEY),
  )

  const initialized = useRef(false)
  const now = useRef(dayjs())
  const setAttendanceInfoModalOpen = attendanceInfoModalState[1]
  ...
  useEffect(() => {
    ...
    if (
      ...
      getStoredFlag(ATTENDANCE_NOTICE_DISMISSED_KEY)
    ) {
      return
    }

    setAttendanceInfoModalOpen(true)
  }, [hasSubmittedAttendance, setAttendanceInfoModalOpen])

  const dismissAttendanceNotice = () => {
    setStoredFlag(ATTENDANCE_NOTICE_DISMISSED_KEY)
    setAttendanceInfoModalOpen(false)
  }

  const markAttendanceSubmitted = () => {
    setStoredFlag(ATTENDANCE_SUBMITTED_KEY)
    setStoredFlag(ATTENDANCE_NOTICE_DISMISSED_KEY)
  ...
  return (
    <>
      ...
      <Modal
        ...
        onClose={dismissAttendanceNotice}
      >
      ...
      >
        <AttendanceFormModal
          ...
          onSubmitted={markAttendanceSubmitted}
        />
      </Modal>
    </>
  )
